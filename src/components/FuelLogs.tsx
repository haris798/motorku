...
export default function FuelLogs({ logs, onAddLog, onEditLog, onDeleteLog, settings }: FuelLogsProps) {
  ...
  const getEfficiencyBadge = (eff: number | undefined) => {
    if (!eff) return <span className="text-[10px] bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400 px-2 py-0.5 rounded-full font-medium">BBM Pertama</span>;
    
    if (eff > 45) {
      return (
        <span className="text-[10px] bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-400 px-2.5 py-1 rounded-full font-bold flex items-center gap-1">
          <Sparkles className="w-3 h-3" /> Sangat Irit ({eff.toFixed(1)} km/L)
        </span>
      );
    } else if (eff > 35) {
      return (
        <span className="text-[10px] bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-400 px-2.5 py-1 rounded-full font-bold flex items-center gap-1">
          Normal ({eff.toFixed(1)} km/L)
        </span>
      );
    } else {
      return (
        <span className="text-[10px] bg-red-100 text-red-800 dark:bg-red-950/40 dark:text-red-400 px-2.5 py-1 rounded-full font-bold flex items-center gap-1">
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Pantau rincian biaya BBM, konsumsi liter, dan kalkulasi efisiensi kendaraan Anda.</p>
        </div>
        <button
          id="btn-add-fuel-log"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-slate-400">Jenis BBM:</span>
          <select
            id="fuel-type-filter"
      ...
      {sortedLogs.length === 0 ? (
        <div className="text-center p-12 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-xs flex flex-col items-center justify-center text-slate-400">
          ...
          <p className="font-bold text-sm">Belum Ada Riwayat BBM</p>
          <p className="text-xs mt-1 max-w-xs">Silakan catat pembelian BBM pertama Anda dengan menekan tombol di atas.</p>
        </div>
      ) : (
          ...
          {sortedLogs.map((log) => (
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-between gap-2 flex-wrap pt-2">
                ...
                {log.notes && (
                  <span className="text-[10px] text-slate-400 max-w-[50%] truncate italic" title={log.notes}>
