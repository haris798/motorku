...
export default function App() {
  const {
    oilLogs, fuelLogs, settings, user,
    activeTab, isOnline, darkMode, syncStatus, syncProgressMsg,
    initStore, setDarkMode, setActiveTab, setOnlineStatus, setUser, triggerSync
  } = useAppStore();
    ...
    if (client) {
      client.auth.getUser().then(async ({ data: { user: sbUser } }) => {
        if (sbUser) {
          ...
          // Auto-login using saved credentials if available (you might want to handle this differently in a real app, storing password locally is not recommended)
  ...
  }, [settings.supabase.url, settings.supabase.anonKey, setUser]);
  ...
  }, [settings.supabase.connected, user, setOnlineStatus, triggerSync]);
    ...
    if (settings.telegram.enabled && oilLogs.length > 0) {
      ...
      checkAndSendOilAlert(
        ...
        if (res.triggered) {
          ...
          // Update setting to avoid duplicate notifications today
  ...
  }, [isOnline, user, settings.supabase.connected, triggerSync]);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-800 dark:text-slate-200 transition-colors duration-300 flex flex-col md:flex-row overflow-hidden pb-16 md:pb-0">
        ...
        <div className="p-4 border-t border-slate-150 dark:border-slate-800">
          <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-2xl border border-slate-150 dark:border-slate-800">
            ...
            <div className="mt-2.5 w-full bg-slate-200 dark:bg-slate-800 h-1 rounded-full overflow-hidden">
              <div
                className={`h-full transition-all duration-500 ${!isOnline
                    ? 'bg-rose-500 w-1/3'
                    : syncStatus.pendingSyncCount > 0
                      ? 'bg-amber-500 w-2/3'
      ...
      <div className="flex-1 flex flex-col h-screen overflow-y-auto min-w-0">
        ...
        <main className="flex-1 p-6 sm:p-8 pb-28 md:pb-8 overflow-y-auto max-w-7xl w-full mx-auto space-y-6">
          ...
          {activeTab === 'oil' && <OilLogs />}
          
          {activeTab === 'fuel' && <FuelLogs />}
